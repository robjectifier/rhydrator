...
from rootfilespec.rntuple.schema import AliasColumnDescription, ColumnDescription, FieldDescription, FIELD_STRUCTURAL_ROLES

# abbott pick up here
# ok, so i can build the field tree structure.
# need to examine what fields are collections/records/variants, and put in special cases there
# naming of parents for those types are weird, address thi


def get_field_to_page_map(schema_description: SchemaDescription, pagelist_envelopes: list[PageListEnvelope], filepath="") -> dict:
    ...
    def get_field_tree(field_descriptions: list[FieldDescription], filepath="") -> dict:
        ...
        def decode_field_flags(field: FieldDescription) -> tuple[str, bool, bool, bool]:
            ...
            return ", ".join(flag_list) if flag_list else "", isRepetitive, isProjected, hasTypeChecksum
        ...
        for field_id, field in enumerate(field_descriptions):
            ...
            field_structural_role = FIELD_STRUCTURAL_ROLES.get(field.fStructuralRole, "Unknown")
            field_flags, isRepetitive, isProjected, hasTypeChecksum = decode_field_flags(field)
            ...
            node = {"name": field_name, "role": field_structural_role, "flags": field_flags, "type": field_type, "children": {}, "projections":{}}
            nodes[field_id] = node
            ...
            if parent_id == field_id:
                ...
                parent_node["children"][field_id] = node
        ...
        if filepath:
            ...
            with open(filepath_field, "w") as f:
                write_tree(tree)
    ...
    def map_columns_to_fields(column_descriptions: list[ColumnDescription], alias_column_descriptions: list[AliasColumnDescription], field_tree: dict, filepath="") -> dict:
        """
        ...
        def decode_column_flags(column: ColumnDescription) -> tuple[str, bool, bool, bool]:
            isDeferred = bool(column.fFlags & 0x1)
            isSuppressed = False # True if isDeferred and first element index is negative
            hasValueRange = bool(column.fFlags & 0x2)
            flag_list = []
            if isDeferred:
                if column.fFirstElementIndex is None:
                    raise ValueError(f"Column with deferred flag set but fFirstElementIndex is None")
                isSuppressed = column.fFirstElementIndex < 0
                flag_list.append(f"Deferred (first element ind: {column.fFirstElementIndex}{', suppressed' if isSuppressed else ''})")
            if hasValueRange:
                flag_list.append(f"Has Value Range (min: {column.fMinValue}, max: {column.fMaxValue})")
            return ", ".join(flag_list) if flag_list else "", isDeferred, isSuppressed, hasValueRange
        ...
        for col_id, column in enumerate(column_descriptions):
            ...
            column_type = column_type.replace("ColumnType.", "") # column_type always starts with "ColumnType.", strip that off for brevity
            column_field_id = column.fFieldID
            column_flags, isDeferred, isSuppressed, hasValueRange = decode_column_flags(column)
            ...
            if column_field_id in field_id_to_node:
                ...
                if "columns" not in node:
                    node["columns"] = []
                node["columns"].append({"column_id": col_id, "type": column_type, "flags": column_flags, "rep_index": column_rep_index})
            else:
        ...
        if filepath:
            ...
            with open(filepath_column, "w") as f:
        ...
        if filepath:
            ...
            with open(filepath_field_column, "w") as f:
    ...
    field_column_tree = map_columns_to_fields(column_descriptions, alias_column_descriptions, field_tree, filepath=filepath)
    ...
    def map_pages_to_columns_to_fields(pagelist_envelope: PageListEnvelope, field_column_tree: dict, field_tree: dict, filepath="") -> dict:
        """
        ...
        for fid, field_info in field_column_tree.items():
        ...
        if filepath:
            ...
            with open(filepath_field_column_page, "w") as f:
                write_field_column_page_tree(field_column_tree)
            print(f"Field to column to page tree written to '{filepath_field_column_page}'")
            ...
            def write_field_column_page_tree_html(node, field_tree, filepath=""):
                ...
                filepath_trimmed = filepath.split('/')[-1].split('.')[0] # Make this the header text
...
"""
                ...
                def build_html_tree(node, indent=0):
                    ...
                    for fid, info in node.items():
                        ...
                        html_lines.append(f'<button class="accordion indent-{indent}" id="{button_id}">{summary}</button>')
                        html_lines.append(f'<div class="panel indent-{indent}" id="{panel_id}">')
                            ...
                            for col in info["columns"]:
                                ...
                                if "cluster_info" in col:
                                    for cid, cinfo in col["cluster_info"].items():
                                        ...
                                        html_lines.append(f"<div style='margin-left: 20px; color: #006666;'>{cluster_str}</div>")
                                        for page in cinfo['pages']:
                                            html_lines.append(f"<div style='margin-left: 40px; font-size: 11px;'>{page}</div>")
                        ...
                        if "alias_columns" in info:
                            ...
                            for acol in info["alias_columns"]:
                                html_lines.append(f"<div>Alias Column - Physical Column ID: {acol['physical_column_id']}</div>")
                            html_lines.append('</div>')
                        ...
                        # Recurse to children
                        if info["children"]:
                            html_lines.extend(build_html_tree(info["children"], indent + 1))
...
"""
            ...
            write_field_column_page_tree_html(field_column_tree, field_tree, filepath=filepath)
    ...
    field_column_page_tree = map_pages_to_columns_to_fields(pagelist_envelopes[0], field_column_tree, field_tree, filepath=filepath)
...
def print_members_recursive(obj):
        ...
        if hasattr(obj, "__dataclass_fields__"):
            ...
            if mode != "suppress_classname":
                ...
                if mode == "in_list":
                    ...
                    lines.append(f"{prefix}{COLOR_CLASS}{obj.__class__.__name__}:{COLOR_RESET}")
            ...
            for f in fields:
                ...
                # If the field is a list or tuple, print each item
                    ...
                    if v:
                        lines[-1] += f"{COLOR_LIST}[{len(v)} item list of {COLOR_CLASS}{v[0].__class__.__name__}{COLOR_LIST}]:{COLOR_RESET}"
    ...
    lines = []
    # Print class header
    lines.append(f"{COLOR_TOP}{'-' * 27} Printing {obj.__class__.__name__}: {'-' * 27}{COLOR_RESET}")
    # Print the members recursively
    _recurse(obj, 0, set(), "", lines)
    # Print class footer 
    lines.append(f"{COLOR_TOP}{'-' * 25} Done Printing {obj.__class__.__name__}! {'-' * 24}\n{COLOR_RESET}")
...
if __name__ == "__main__":
    ...
    # filename = "rntuples/DYJetsToLL_RunIIAutumn18NanoAODv7_RNTuple"
    # path = Path(f"{filename}.root")

    # filename = "rntuples/initialconversion_rntuple_mini24_lzma4_buffer4"
    filename = "rntuples/initialconversion_rntuple_minisim24_lzma4_buffer4"
    ...
    with path.open("rb") as filehandle:
        ...
        for name, tkey in keylist.items():
            ...
            if tkey.fClassName.fString == b"ROOT::RNTuple":
                ...
                for fieldID, fieldDescription in enumerate(schemaDescription.fieldDescriptions):
                ...
                print(f"\tTotal Top-Level Fields: {len(topLevelFields_names)} / {nFields}")

                # Get field tree structure
                field_tree = get_field_to_page_map(schemaDescription, rntuple.pagelistEnvelopes, filepath=f"{filename}_{name}")
                # print(field_tree)
